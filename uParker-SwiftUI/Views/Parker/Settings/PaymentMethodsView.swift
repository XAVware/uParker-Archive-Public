...
extension PaymentMethodsView {
    @MainActor class PaymentMethodsViewModel: ObservableObject {
        ...
        @Published var isAddingManually: Bool = false

        enum AddMethod { case licensePlate, vin, manual }
...
//                    addLicensePlateView
        ...
        .navigationTitle(vm.isAddingManually ? "Your Vehicles" : "Add Vehicle")
...
//                        Text(self.licensePlate)
    ...
    private var addButtonPanel: some View {
        VStack(alignment: .leading) {
            ...
            Button {
                withAnimation { vm.selectedMethod = .vin }
            } label: {
                Text("VIN")
                    .frame(maxWidth: .infinity)
            }
